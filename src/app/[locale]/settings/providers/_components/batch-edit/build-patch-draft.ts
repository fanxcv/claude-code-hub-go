...
export function buildPatchDraftFromFormState(
  ...
  if (dirtyFields.has("routing.codexMaxTokensPreference")) {
    if (state.routing.codexMaxTokensPreference === "inherit") {
      draft.codex_max_tokens_preference = { clear: true };
    } else {
      draft.codex_max_tokens_preference = { set: state.routing.codexMaxTokensPreference };
    }
  }
  ...
  // OpenAI-compatible preferences
  if (dirtyFields.has("routing.openaiMaxTokensPreference")) {
    if (state.routing.openaiMaxTokensPreference === "inherit") {
      draft.openai_max_tokens_preference = { clear: true };
    } else {
      draft.openai_max_tokens_preference = { set: state.routing.openaiMaxTokensPreference };
    }
  }
  ...
  // 等待阶梯两项（递增时长：分钟 -> 毫秒；清空 = 不启用阶梯）
  if (dirtyFields.has("circuitBreaker.releaseIncrementMinutes")) {
    if (state.circuitBreaker.releaseIncrementMinutes === null) {
      draft.circuit_breaker_release_increment = { clear: true };
    } else {
      draft.circuit_breaker_release_increment = {
        set: state.circuitBreaker.releaseIncrementMinutes * 60000,
      };
    }
  }
  if (dirtyFields.has("circuitBreaker.maxOpenCount")) {
    if (state.circuitBreaker.maxOpenCount === null) {
      draft.circuit_breaker_max_open_count = { clear: true };
    } else {
      draft.circuit_breaker_max_open_count = { set: state.circuitBreaker.maxOpenCount };
    }
  }
