...
export interface BatchSettingsAnalysis {
  routing: {
    ...
    codexMaxTokensPreference: FieldAnalysisResult<CodexMaxTokensPreference>;
    ...
    openaiMaxTokensPreference: FieldAnalysisResult<OpenAIMaxTokensPreference>;
  ...
  circuitBreaker: {
    ...
    // 等待阶梯两项：分钟（与表单同单位）/ 次数；null = 不启用阶梯。
    releaseIncrementMinutes: FieldAnalysisResult<number | null>;
    maxOpenCount: FieldAnalysisResult<number | null>;
  };
  network: {
...
export function analyzeBatchProviderSettings(providers: ProviderDisplay[]): BatchSettingsAnalysis {
  return {
    routing: {
      ...
      codexMaxTokensPreference: analyzeField(
        providers,
        (p) => p.codexMaxTokensPreference ?? "inherit"
      ),
      ...
      openaiMaxTokensPreference: analyzeField(
        providers,
        (p) => p.openaiMaxTokensPreference ?? "inherit"
      ),
    ...
    circuitBreaker: {
      ...
      releaseIncrementMinutes: analyzeField(providers, (p) =>
        p.circuitBreakerReleaseIncrement ? p.circuitBreakerReleaseIncrement / 60000 : null
      ),
      maxOpenCount: analyzeField(providers, (p) => p.circuitBreakerMaxOpenCount ?? null),
    },
    network: {
