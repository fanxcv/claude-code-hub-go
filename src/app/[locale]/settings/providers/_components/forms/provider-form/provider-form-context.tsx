...
const ACTION_TO_FIELD_PATH: Partial<Record<ProviderFormActionWith5hResetMode["type"], string>> = {
  ...
  SET_PROTOCOL_CONVERSION_ENABLED: "routing.protocolConversionEnabled",
  ...
  SET_CODEX_MAX_TOKENS: "routing.codexMaxTokensPreference",
  SET_ANTHROPIC_MAX_TOKENS: "routing.anthropicMaxTokensPreference",
  SET_OPENAI_MAX_TOKENS: "routing.openaiMaxTokensPreference",
  ...
  SET_RELEASE_INCREMENT_MINUTES: "circuitBreaker.releaseIncrementMinutes",
  SET_MAX_OPEN_COUNT: "circuitBreaker.maxOpenCount",
...
export function createInitialState(
    ...
    return {
      ...
      routing: {
        ...
        codexMaxTokensPreference:
          analysis.routing.codexMaxTokensPreference.status === "uniform"
            ? analysis.routing.codexMaxTokensPreference.value
            : "inherit",
        ...
        openaiMaxTokensPreference:
          analysis.routing.openaiMaxTokensPreference.status === "uniform"
            ? analysis.routing.openaiMaxTokensPreference.value
            : "inherit",
      ...
      circuitBreaker: {
        ...
        maxRetryAttempts:
          analysis.circuitBreaker.maxRetryAttempts.status === "uniform"
            ...
            : null,
        // 等待阶梯：批量入口不把「混合值」当启用（拿不准就不改它）——null 即不启用阶梯。
        releaseIncrementMinutes:
          analysis.circuitBreaker.releaseIncrementMinutes.status === "uniform"
            ? analysis.circuitBreaker.releaseIncrementMinutes.value
            : null,
        maxOpenCount:
          analysis.circuitBreaker.maxOpenCount.status === "uniform"
            ? analysis.circuitBreaker.maxOpenCount.value
            : null,
      },
      network: {
        proxyUrl:
  ...
  return {
    ...
    routing: {
      ...
      codexMaxTokensPreference: sourceProvider?.codexMaxTokensPreference ?? "inherit",
      ...
      openaiMaxTokensPreference: sourceProvider?.openaiMaxTokensPreference ?? "inherit",
    ...
    circuitBreaker: {
      ...
      // 阶梯两项：库列可空且无默认，null 即「不启用阶梯」。
      releaseIncrementMinutes: sourceProvider?.circuitBreakerReleaseIncrement
        ? sourceProvider.circuitBreakerReleaseIncrement / 60000
        : null,
      maxOpenCount: sourceProvider?.circuitBreakerMaxOpenCount ?? null,
    },
    network: {
...
export function providerFormReducer(
  ...
  switch (action.type) {
    ...
    case "SET_PROTOCOL_CONVERSION_ENABLED":
      return {
        ...state,
    ...
    case "SET_CODEX_MAX_TOKENS":
      return {
        ...state,
        routing: { ...state.routing, codexMaxTokensPreference: action.payload },
      };
    ...
    case "SET_OPENAI_MAX_TOKENS":
      return {
        ...state,
        routing: { ...state.routing, openaiMaxTokensPreference: action.payload },
      };
    ...
    case "SET_RELEASE_INCREMENT_MINUTES":
      return {
        ...state,
        circuitBreaker: { ...state.circuitBreaker, releaseIncrementMinutes: action.payload },
      };
    case "SET_MAX_OPEN_COUNT":
      return {
        ...state,
        circuitBreaker: { ...state.circuitBreaker, maxOpenCount: action.payload },
      };
