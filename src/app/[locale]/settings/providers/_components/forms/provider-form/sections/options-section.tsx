      ...
      >
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              >
                <Switch
                  ...
                  aria-label={t("sections.routing.preserveClientIp.label")}
              ...
              >
                <Switch
                  ...
                  aria-label={t("sections.routing.disableSessionReuse.label")}
              ...
              {/* 协议转换开关 - 仅对 claude/claude-auth/codex/openai-compatible 有意义（gemini 两线不走协议转换）；批量模式不暴露，见 provider-form-context 注释 */}
              {!isBatch && providerType !== "gemini" && providerType !== "gemini-cli" && (
                <ToggleRow
                  label={t("sections.routing.protocolConversion.label")}
                  description={t("sections.routing.protocolConversion.desc")}
                >
                  <Switch
                    id={isEdit ? "edit-protocol-conversion" : "protocol-conversion"}
                    aria-label={t("sections.routing.protocolConversion.label")}
                    checked={state.routing.protocolConversionEnabled}
                    onCheckedChange={(checked) =>
                      dispatch({ type: "SET_PROTOCOL_CONVERSION_ENABLED", payload: checked })
                    }
                    disabled={state.ui.isPending}
                  />
                </ToggleRow>
              ...
              >
                <Switch
                  ...
                  aria-label={t("sections.routing.swapCacheTtlBilling.label")}
          ...
          {(providerType === "codex" || isBatch) && (
            ...
            >
              <div className="space-y-4">
                <SmartInputWrapper label={t("sections.routing.codexOverrides.maxTokens.label")}>
                  <div className="flex gap-2">
                    <Select
                      value={
                        state.routing.codexMaxTokensPreference === "inherit" ? "inherit" : "custom"
                      }
                      onValueChange={(val) => {
                        if (val === "inherit") {
                          dispatch({ type: "SET_CODEX_MAX_TOKENS", payload: "inherit" });
                        } else {
                          dispatch({ type: "SET_CODEX_MAX_TOKENS", payload: "8192" });
                        }
                      }}
                      disabled={state.ui.isPending}
                    >
                      <SelectTrigger
                        className={
                          state.routing.codexMaxTokensPreference === "inherit"
                            ? "flex-1 min-w-0"
                            : "w-40"
                        }
                      >
                      ...
                      <SelectContent>
                        <SelectItem value="inherit">
                          {t("sections.routing.codexOverrides.maxTokens.options.inherit")}
                        </SelectItem>
                        <SelectItem value="custom">
                          {t("sections.routing.codexOverrides.maxTokens.options.custom")}
                        </SelectItem>
                      </SelectContent>
                    </Select>
                    {state.routing.codexMaxTokensPreference !== "inherit" && (
                      <Input
                        ...
                        value={
                          state.routing.codexMaxTokensPreference === "inherit"
                            ? ""
                            : state.routing.codexMaxTokensPreference
                        }
                        onChange={(e) => {
                          ...
                          if (val === "") {
                            ...
                            dispatch({ type: "SET_CODEX_MAX_TOKENS", payload: val });
                          }
                        }}
                        placeholder={t("sections.routing.codexOverrides.maxTokens.placeholder")}
                        disabled={state.ui.isPending}
                        min="1"
                        max="64000"
                        className="flex-1"
          ...
          {/* OpenAI-compatible Overrides - OpenAI-compatible type only (or batch mode) */}
          {(providerType === "openai-compatible" || isBatch) && (
            <SectionCard
              title={t("sections.routing.openaiOverrides.title")}
              description={t("sections.routing.openaiOverrides.desc")}
              icon={Timer}
              badge={
                isBatch ? (
                  <Badge variant="outline">{tBatch("batchNotes.openaiOnly")}</Badge>
                ) : undefined
              }
            >
              <div className="space-y-4">
                <SmartInputWrapper label={t("sections.routing.openaiOverrides.maxTokens.label")}>
                  <div className="flex gap-2">
                    <Select
                      value={
                        state.routing.openaiMaxTokensPreference === "inherit" ? "inherit" : "custom"
                      }
                      onValueChange={(val) => {
                        if (val === "inherit") {
                          dispatch({ type: "SET_OPENAI_MAX_TOKENS", payload: "inherit" });
                        } else {
                          dispatch({ type: "SET_OPENAI_MAX_TOKENS", payload: "8192" });
                        }
                      }}
                      disabled={state.ui.isPending}
                    >
                      <SelectTrigger
                        className={
                          state.routing.openaiMaxTokensPreference === "inherit"
                            ? "flex-1 min-w-0"
                            : "w-40"
                        }
                      >
                      ...
                      <SelectContent>
                        <SelectItem value="inherit">
                          {t("sections.routing.openaiOverrides.maxTokens.options.inherit")}
                        </SelectItem>
                        <SelectItem value="custom">
                          {t("sections.routing.openaiOverrides.maxTokens.options.custom")}
                        </SelectItem>
                      </SelectContent>
                    </Select>
                    {state.routing.openaiMaxTokensPreference !== "inherit" && (
                      <Input
                        ...
                        value={
                          state.routing.openaiMaxTokensPreference === "inherit"
                            ? ""
                            : state.routing.openaiMaxTokensPreference
                        }
                        onChange={(e) => {
                          const val = e.target.value;
                          if (val === "") {
                            ...
                            dispatch({ type: "SET_OPENAI_MAX_TOKENS", payload: val });
                          }
                        }}
                        placeholder={t("sections.routing.openaiOverrides.maxTokens.placeholder")}
                        disabled={state.ui.isPending}
                        min="1"
                        max="64000"
                        className="flex-1"
              ...
              <div className="space-y-4">
                ...
                >
                  <Switch
                    ...
                    aria-label={t("sections.routing.activeTime.toggleLabel")}
                    checked={
                ...
                <div className="space-y-4">
                  ...
                  {state.routing.slowRateMonitorEnabled && (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      <SmartInputWrapper
                        label={t("sections.routing.slowRate.probeAfterFirstByteSeconds.label")}
                        description={t("sections.routing.slowRate.probeAfterFirstByteSeconds.desc")}
                      >
                        <Input
                          id={
                            isEdit
                              ? "edit-slow-rate-probe-after-first-byte"
                              : "slow-rate-probe-after-first-byte"
                          }
                          type="number"
                          value={state.routing.slowRateProbeAfterFirstByteSeconds ?? ""}
                          onChange={(e) =>
                            dispatch({
                              type: "SET_SLOW_RATE_PARAMS",
                              payload: {
