...
function ProviderFormContent({
    ...
    startTransition(async () => {
      try {
        ...
        const openDurationMs = state.circuitBreaker.openDurationMinutes
          ...
          : undefined;
        // 等待阶梯的递增时长也存毫秒（与 circuit_breaker_open_duration 同单位）；
        // 空 = 不启用阶梯（null，与库列的可空无默认一致）。
        const releaseIncrementMs = state.circuitBreaker.releaseIncrementMinutes
          ? state.circuitBreaker.releaseIncrementMinutes * 60 * 1000
          : null;
        ...
        const baseFormData = {
          ...
          codex_max_tokens_preference: state.routing.codexMaxTokensPreference,
          ...
          openai_max_tokens_preference: state.routing.openaiMaxTokensPreference,
          ...
          circuit_breaker_release_increment: releaseIncrementMs,
          circuit_breaker_max_open_count: state.circuitBreaker.maxOpenCount,
  ...
  const tabStatus = useMemo((): Partial<Record<TabId, "default" | "warning" | "configured">> => {
    ...
    if (
      ...
      state.routing.protocolConversionEnabled ||
