...
interface UsersPageClientProps {
  ...
  currentUser: UsersPageCurrentUser;
}

/**
 * 当前用户在本页客户端侧的可用字段。
 *
 * 静态化改造后会话来自 `__CCH_BOOTSTRAP__`（`UiSessionSnapshot.user`），只有 `id/name/role`，
 * 不再是服务端 `getSession()` 返回的完整 `User` 行。本页只读 `id`（过滤出自己的行）与
 * `role`（判管理员）；下游 `UserManagementTable` 同样只读 `role`
 * （`user-management-table.tsx:137`），其透传给 `UserKeyTableRow` 的 `currentUser` 形参
 * （`{ role: string }`）在该文件里**未被消费**。
 *
 * 若将来有消费方开始读其它字段，必须改为从 REST 取完整用户行，而不是继续依赖本收窄类型。
 */
export type UsersPageCurrentUser = {
  id: number;
  name?: string;
  role?: "admin" | "user";
};
      ...
      {isInitialLoading ? (
        ...
        <div className="space-y-3">
          <UserManagementTable
            ...
            /* 该形参声明为完整的 `User`（本目录不可改：见 user-management-table.tsx:26）。
               `User` 可赋给收窄的 `UsersPageCurrentUser`，反向断言合法；消费链只读 role。 */
