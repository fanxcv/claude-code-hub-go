...
interface UsageLogsViewVirtualizedProps {
  ...
  /**
   * 刷新间隔的**默认值覆盖**（毫秒；本机偏好未设置时生效）。使用记录页现在由
   * `useLogsRefreshPreference` 管理间隔，用户在页面上选的值存 localStorage。
   * 这个 prop 留给「部署侧想改出厂默认」时接入（如 Go 壳注入的环境变量），
   * 未提供时用 `DEFAULT_LOGS_REFRESH_INTERVAL_MS`（3 秒）。
   */
...
function UsageLogsViewContent({
  ...
  // 自动刷新偏好（模式 + 间隔）：本机偏好，存 localStorage；间隔为 0 即「关闭」。
  const refresh = useLogsRefreshPreference(userId, logsRefreshIntervalMs);
  const [streamStatus, setStreamStatus] = useState<UsageLogsStreamStatus>("idle");
  // 向下翻历史时暂停自动刷新（避免把用户正在看的位置顶走），由表格上报。
  const [isHistoryBrowsing, setIsHistoryBrowsing] = useState(false);
  const handleHistoryBrowsingChange = useCallback((browsing: boolean) => {
    setIsHistoryBrowsing(browsing);
  }, []);
  const handleStreamStatusChange = useCallback((status: UsageLogsStreamStatus) => {
    setStreamStatus(status);
  }, []);
  // 开关关掉前的非零间隔：重新打开时回到用户上次选择，而不是回落到出厂值。
  const lastIntervalRef = useRef(DEFAULT_LOGS_REFRESH_INTERVAL_MS);
  useEffect(() => {
    if (refresh.intervalMs > 0) lastIntervalRef.current = refresh.intervalMs;
  }, [refresh.intervalMs]);
  ...
  // 自动刷新引擎：拉取模式按间隔取新行，推送模式订阅 SSE 信号后取新行；两者都另有
  // 首页兜底重取（覆盖在途行的用量/计费落库）。间隔为 0（关闭）或正在翻历史时不自刷。
  //
  // 全屏内与全屏外由**同一个**引擎驱动：两个表格读同一份 query 缓存，全屏只是换个展示容器；
  // 若各挂一份轮询，会回到「每 tick 重取所有页」的老行为，并可能双写同一缓存。
  const logsQueryKey = useMemo(() => [USAGE_LOGS_QUERY_KEY_PREFIX, filters], [filters]);
  const autoRefreshEnabled = refresh.intervalMs > 0 && !isHistoryBrowsing;
  useLogsRefreshEngine({
  ...
  const handleAutoRefreshToggle = useCallback(
    (next: boolean) => {
      refresh.setIntervalMs(next ? lastIntervalRef.current : 0);
    },
    [refresh]
  );
  ...
  const handleManualRefresh = useCallback(async () => {
    ...
    // 清回首页再重取（而非 invalidate）：invalidate 会留下「新页 0 + 旧页 1」，两者之间的行
    // 两边都不覆盖，滚动时会跳过一段。理由与代价见 resetUsageLogsPages 的注释。
    await resetUsageLogsPages(queryClientInstance);
      ...
      <div className="space-y-3">
        ...
        {hasStatsFilters && (
          <UsageLogsStatsPanel
            ...
            autoRefreshIntervalMs={autoRefreshEnabled ? STATS_REFRESH_INTERVAL_MS : 0}
          />
        )}
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1">
              ...
              <LogsRefreshControl
                mode={refresh.mode}
                intervalMs={refresh.intervalMs}
                onModeChange={refresh.setMode}
                onIntervalChange={refresh.setIntervalMs}
        ...
        <div className="rounded-lg border border-border/60 overflow-hidden">
          <VirtualizedLogsTable
            ...
            serverTimeZone={serverTimeZone}
            onHistoryBrowsingChange={handleHistoryBrowsingChange}
          />
        </div>
      </div>
