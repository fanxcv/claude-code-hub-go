...
export function SummaryTab({
  ...
  const effortDisplay = thinkingEffortInfo
    ? {
        ...
        // 归因分开：协议转换改写了值就说转换（给出协议对），供应商覆写才说覆写——
        // 把前者写成「已被供应商覆写」是错归因（两条来源本就不是一回事）。
        conversionPair:
          thinkingEffortInfo.conversion?.converted === true &&
          thinkingEffortInfo.conversion.clientProtocol != null &&
          thinkingEffortInfo.conversion.targetProtocol != null
            ? `${thinkingEffortInfo.conversion.clientProtocol} → ${thinkingEffortInfo.conversion.targetProtocol}`
            : null,
        dropped: thinkingEffortInfo.conversion?.dropped === true,
        droppedText: t("effortConversion.dropped"),
        droppedTooltip: t("effortConversion.droppedTooltip"),
      }
    : null;
              ...
              <div className="p-4">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {effortDisplay.dropped && (
                    ...
                    >
                      {effortDisplay.droppedText}
                    </Badge>
                  )}
                </div>
                {effortDisplay.conversionPair && (
                ...
                {effortDisplay.dropped && (
                  <p className="text-[11px] text-amber-600 dark:text-amber-400 mt-1">
                    {effortDisplay.droppedTooltip}
                  </p>
                )}
                {thinkingEffortInfo?.isProviderOverridden && (
