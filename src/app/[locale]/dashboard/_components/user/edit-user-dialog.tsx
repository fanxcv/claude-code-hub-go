...
function EditUserDialogInner({ onOpenChange, user, onSuccess }: EditUserDialogProps) {
  ...
  // 「仅重置 5H 限额」的失败原因现在是 REST 的 errorCode（如 USER_5H_LIMIT_NOT_CONFIGURED）。
  // 这些码与 Node `ERROR_CODES` 同名，且 `errors` 命名空间里有对应文案，故优先本地化，
  // 拿不到才回落服务端 detail——否则会展示 Problem 的英文 title。
  ...
  const handleReset5hLimitOnly = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(
          code && tErrors.has(code) ? tErrors(code) : res.error || t("editDialog.reset5h.error")
        );
        return;
      }
