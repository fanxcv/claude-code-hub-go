...
export function UserUnlimitedItem({ user, currencyCode = "USD" }: UserUnlimitedItemProps) {
  ...
  const topKeys = [...user.keys]
    .sort(
      (a, b) => b.todayUsage - a.todayUsage || compareTotalUsageDesc(a.totalUsage, b.totalUsage)
    )
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="text-right space-y-1">
            ...
            <p className="text-lg font-semibold">
              {formatTotalUsage(user.totalUsage, currencyCode, t("totalUsageUnavailable"))}
            </p>
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
          ...
          {topKeys.map((key) => (
            <KeyChip
              ...
              keyData={key}
              currencyCode={currencyCode}
              unavailableLabel={t("totalUsageUnavailable")}
            />
          ))}
