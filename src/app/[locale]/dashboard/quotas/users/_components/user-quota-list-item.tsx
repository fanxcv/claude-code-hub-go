...
import { compareTotalUsageDesc, formatTotalUsage } from "./total-usage";
...
function KeyPreview({
  ...
  return (
    <Badge variant="secondary" className="font-mono text-xs">
      {keyData.name} · {formatTotalUsage(keyData.totalUsage, currencyCode, unavailableLabel)}
    </Badge>
  );
}
...
export function UserQuotaListItem({ user, currencyCode = "USD" }: UserQuotaListItemProps) {
  ...
  const sortedKeys = useMemo(() => {
    return [...user.keys].sort((a, b) => {
      if (b.todayUsage === a.todayUsage) {
        // 累计成本未知时排在已知之后（不把「读不到」当成 0 参与排序）。
        return compareTotalUsageDesc(a.totalUsage, b.totalUsage);
      }
  ...
  const unavailableLabel = t("totalUsageUnavailable");
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="text-right space-y-1">
            ...
            <p className="text-lg font-semibold">
              {formatTotalUsage(totalUsage, currencyCode, unavailableLabel)}
            </p>
          </div>
        </div>
          ...
          <div className="text-sm space-y-1">
            ...
            {totalLimit && totalLimit > 0 ? (
              <div className="space-y-1">
                ...
                {totalUsage === null ? (
                  // 读数不可用时**不画进度条**：0% 会被读成「额度充足」，而真相是「不知道」。
                  <p className="text-xs text-muted-foreground">{unavailableLabel}</p>
                ) : (
                  <>
                    <QuotaProgress current={totalUsage} limit={totalLimit} className="h-2" />
                    <p className="text-xs text-muted-foreground">
                  ...
                  {remainingKeys.map((key: UserKeyWithUsage) => (
                    ...
                    >
                      ...
                      <span className="text-muted-foreground">
                        {t("totalCost")}:{" "}
                        {formatTotalUsage(key.totalUsage, currencyCode, unavailableLabel)}
                      </span>
                      <Dot className="h-4 w-4 text-muted-foreground" />
                      <span className="text-muted-foreground">
