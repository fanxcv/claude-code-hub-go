...
export type { AvailableModelCatalogItem, AvailableModelCatalogScope } from "@/types/model-price";
export type {
  EditProviderResult,
...
// 熔断日志：熔断状态 + 该供应商近期错误（排查用）。
//
// 与 getProvidersHealthStatus 同样**不**用 toActionResult 包装：调用方（弹窗）需要直接拿到数据，
// 且失败时应由弹窗自己呈现降级（本端点即便 Redis 或库不可用也返回 200 + 明确的 available/
// errorsUnavailableReason，故这里是真异常才算失败）。
export function getProviderCircuitLogs(
  providerId: number,
  limit?: number
): Promise<ProviderCircuitLogs> {
