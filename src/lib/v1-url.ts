...
/**
 * 在供应商 base URL 上拼接「指定端点路径」的上游 URL。
 *
 * 协议转换把客户端端点改写成目标协议线的端点（如 `/v1/responses` → `/v1/chat/completions`），
 * 此时路径**必须**经本函数拼接，不能直接 `new URL(proxyUrl).pathname = p`：后者是整体替换，
 * 会把 base URL 自带的路径前缀（如 `https://host/zen/go`）连同版本段一起丢掉，
 * 请求会打到 `https://host/v1/...` 而返回 404。
 *
 * @param baseUrl - 供应商 base URL（可含路径前缀）
 * @param requestUrl - 客户端原始请求 URL（保留其 query）
 * @param pathname - 目标端点路径（上游协议线下的绝对路径）
 */
export function buildProxyUrlForPath(baseUrl: string, requestUrl: URL, pathname: string): string {
  const rewritten = new URL(requestUrl.toString());
  rewritten.pathname = pathname;
  return buildProxyUrl(baseUrl, rewritten);
}
