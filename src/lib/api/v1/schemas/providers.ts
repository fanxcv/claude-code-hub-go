...
export const ProviderSummarySchema = z
  .object({
    ...
    protocolConversionEnabled: z
      .boolean()
      .describe("Whether cross-protocol client requests are accepted for this provider."),
    ...
    // 等待阶梯：窗口 = 熔断时长 + 递增时长 × 级数（级数封顶最大次数）。
    // 两者都为 null / 非正即不启用阶梯（窗口恒为熔断时长）。
    circuitBreakerReleaseIncrement: z
      .number()
      .int()
      .nullable()
      .describe("Circuit breaker backoff ladder increment in milliseconds."),
    circuitBreakerMaxOpenCount: z
      .number()
      .int()
      .nullable()
      .describe("Circuit breaker backoff ladder maximum level."),
    ...
    codexMaxTokensPreference: z.string().nullable().describe("Codex max output tokens preference."),
    ...
    openaiMaxTokensPreference: z
      .string()
      .nullable()
      .describe("OpenAI-compatible max tokens preference."),
...
export const ProviderCreateSchema = z
  .object({
    ...
    circuit_breaker_half_open_success_threshold: z
      ...
      .describe("Circuit breaker half-open success threshold."),
    // 等待阶梯：窗口 = 熔断时长 + 递增时长 × 级数（级数封顶最大次数）。
    // 两者均可为 null 或省略 = 不启用阶梯（窗口恒为熔断时长）。
    circuit_breaker_release_increment: z
      .number()
      .int()
      .min(0)
      .max(86_400_000)
      ...
      .optional()
      .describe("Circuit breaker backoff ladder increment in milliseconds."),
    circuit_breaker_max_open_count: z
      .number()
      .int()
      .min(0)
      .max(100)
    ...
    codex_max_tokens_preference: z
      .string()
      .optional()
      .describe("Codex max output tokens preference."),
    ...
    anthropic_adaptive_thinking: z
      ...
      .optional()
      .describe("Anthropic adaptive thinking config."),
    openai_max_tokens_preference: z
      .string()
      .optional()
      .describe("OpenAI-compatible max tokens preference."),
