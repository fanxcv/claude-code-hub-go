...
vi.mock("@/lib/api-client/v1/actions/provider-endpoints", () => providerEndpointsActionMocks);
...
// 熔断徽标的分叉回归：生产实报「页面显示已熔断、请求却照常通过」。
// 口径：open = 真拦截（徽标「熔断」）；half-open = 已过期在放行试探（徽标「恢复中」）；
// closed = 无徽标。三者必须能区分——把半开画成熔断就是用户看到的那个错。
describe("ProviderRichListItem Circuit Badge", () => {
  beforeEach(() => {
  ...
  const renderWithState = async (circuitState: "closed" | "open" | "half-open") => {
    const provider = makeProviderDisplay();
    const { unmount } = renderWithProviders(
      <ProviderRichListItem
        provider={provider}
    ...
    await flushTicks(3);
    return { unmount, text: document.body.textContent ?? "" };
  };

  test("open 显示「熔断」徽标", async () => {
    const { unmount, text } = await renderWithState("open");
    expect(text).toContain("Key Circuit");
    expect(text).not.toContain("Circuit recovering");
    unmount();
  });

  test("half-open 显示「恢复中」而不是「熔断」", async () => {
    const { unmount, text } = await renderWithState("half-open");
    expect(text).toContain("Circuit recovering");
    expect(text).not.toContain("Key Circuit");
    unmount();
  });

  test("closed 两个徽标都不显示", async () => {
    const { unmount, text } = await renderWithState("closed");
    expect(text).not.toContain("Key Circuit");
    expect(text).not.toContain("Circuit recovering");
    unmount();
  });
