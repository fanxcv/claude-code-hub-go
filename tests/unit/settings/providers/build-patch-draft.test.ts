...
// ---------------------------------------------------------------------------
// 熔断等待阶梯（circuit backoff ladder）
//
// 公式：窗口 = 基础时长 + 递增时长 × 级数（级数封顶最大次数）。表单里递增时长用**分钟**，
// 而库列存**毫秒**（与 circuit_breaker_open_duration 同单位），两处口径必须对齐；
// 清空 = 不启用阶梯，对应 patch 的 clear（库列可空无默认）。
// ---------------------------------------------------------------------------

describe("buildPatchDraftFromFormState - 熔断等待阶梯", () => {
  it("递增时长由分钟转毫秒（与熔断时长同单位）", () => {
    const state = createBatchState();
    state.circuitBreaker.releaseIncrementMinutes = 10;
    const dirty = new Set(["circuitBreaker.releaseIncrementMinutes"]);

    const draft = buildPatchDraftFromFormState(state, dirty);

    expect(draft.circuit_breaker_release_increment).toEqual({ set: 600000 });
  });

  it("最大次数按次数原样写入", () => {
    const state = createBatchState();
    state.circuitBreaker.maxOpenCount = 3;
    const dirty = new Set(["circuitBreaker.maxOpenCount"]);

    const draft = buildPatchDraftFromFormState(state, dirty);

    expect(draft.circuit_breaker_max_open_count).toEqual({ set: 3 });
  });

  it("清空两项 => clear（= 不启用阶梯，而不是写 0）", () => {
    const state = createBatchState();
    state.circuitBreaker.releaseIncrementMinutes = null;
    state.circuitBreaker.maxOpenCount = null;
    const dirty = new Set([
      "circuitBreaker.releaseIncrementMinutes",
      "circuitBreaker.maxOpenCount",
    ]);

    const draft = buildPatchDraftFromFormState(state, dirty);

    expect(draft.circuit_breaker_release_increment).toEqual({ clear: true });
    expect(draft.circuit_breaker_max_open_count).toEqual({ clear: true });
  });

  it("未触碰这两项时不写进 patch（默认不启用，不打扰别的供应商）", () => {
    const state = createBatchState();
    const draft = buildPatchDraftFromFormState(state, new Set(["batch.isEnabled"]));

    expect(draft.circuit_breaker_release_increment).toBeUndefined();
    expect(draft.circuit_breaker_max_open_count).toBeUndefined();
  });
});
