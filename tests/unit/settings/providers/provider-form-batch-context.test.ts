...
describe("createInitialState - batch mode", () => {
  ...
  // 等待阶梯的出厂默认是**不启用**（用户明确裁决：默认 0 = 关，行为与加该特性之前一致）。
  it("returns disabled backoff ladder by default", () => {
    const state = createInitialState("batch");

    expect(state.circuitBreaker.releaseIncrementMinutes).toBeNull();
    expect(state.circuitBreaker.maxOpenCount).toBeNull();
  });
