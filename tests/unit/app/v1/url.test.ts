...
import { buildProxyUrl, buildProxyUrlForPath } from "@/lib/v1-url";
...
describe("buildProxyUrlForPath", () => {
  const request = (path: string) => new URL(`https://gw.example.com${path}`);

  test("协议转换重写端点时必须保留 base URL 自带的路径前缀", () => {
    // 回归：曾经直接 new URL(proxyUrl).pathname = p，整体替换把 /zen/go 吃掉，
    // 实际打到 https://opencode.ai/v1/chat/completions 而返回 404 HTML。
    expect(
      buildProxyUrlForPath(
  ...
  test("保留客户端原 query", () => {
    expect(
      buildProxyUrlForPath(
        "https://opencode.ai/zen/go/",
        request("/v1/responses?trace=1"),
        "/v1/chat/completions"
      )
    ).toBe("https://opencode.ai/zen/go/v1/chat/completions?trace=1");
  });

  test("base URL 无路径时退化为普通拼接", () => {
    expect(
      buildProxyUrlForPath(
        "http://hc.example.com:4321",
        request("/v1/responses"),
        "/v1/chat/completions"
      )
    ).toBe("http://hc.example.com:4321/v1/chat/completions");
  });

  test("base URL 已含版本根路径时不重复拼接版本段", () => {
    expect(
      buildProxyUrlForPath(
    ...
    ).toBe("https://relay.example.com/proxy/v1/chat/completions");
  });
});
