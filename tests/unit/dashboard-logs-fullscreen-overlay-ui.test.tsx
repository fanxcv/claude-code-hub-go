...
// useQueryClient 的 mock 必须覆盖被测 hook 真正调用的 API：`logs-refresh` 的自动刷新引擎会先
// `getQueryData` 读首页游标（缓存为空则本轮不发请求）。缺这个方法会让调用点在 effect 里抛
// TypeError，而 effect 里的异常会被 Vitest 报成 **unhandled rejection**（不是用例失败，会掩盖断言）。
