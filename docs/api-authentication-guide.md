...
CC Hub Go 的新版管理 API 位于 `/api/v1/*`，与代理 API `/v1/*` 互相独立。
（历史上的 Server Action 适配层 `/api/actions/*` 已随 Node 后端一并退役，本后端不再提供该面。）
...
- GitHub 仓库：`https://github.com/fanxcv/claude-code-hub-go`
