...
show_help() {
    cat << EOF
CC Hub Go - One-Click Deployment Script v${VERSION}
...
For more information, visit: https://github.com/fanxcv/claude-code-hub-go
EOF
}
...
print_header() {
    ...
    echo "+=================================================================+"
    echo "|                                                                 |"
    echo "|                CC Hub Go - One-Click Deployment                 |"
...
services:
  ...
  app:
    image: ghcr.io/fanxcv/claude-code-hub-go:${IMAGE_TAG}
...
EOF
    ...
    networks:
      - claude-code-hub-net-${SUFFIX}
    healthcheck:
      # 运行镜像里没有 curl（alpine 基座只带 busybox），故用 wget；路径为 Go 侧真实端点
      # （旧的 Node 端点 /api/actions/health 已随 Node 退役删除）。
      test: ["CMD-SHELL", "wget -q -O /dev/null http://localhost:\${APP_PORT:-${APP_PORT}}/api/health || exit 1"]
      interval: 30s
      timeout: 5s
      retries: 3
      start_period: 30s
...
print_success_message() {
    ...
    if [[ "$UPDATE_MODE" == true ]]; then
        echo -e "${GREEN}|                CC Hub Go Updated Successfully!                 |${NC}"
    else
        echo -e "${GREEN}|                CC Hub Go Deployed Successfully!                |${NC}"
    fi
    echo -e "${GREEN}|                                                                |${NC}"
    echo -e "${GREEN}+================================================================+${NC}"
    echo ""
