...
func TestStreamDrainBudgetKeepsMeteringAfterClientAbort(t *testing.T) {
	release := make(chan struct{})
	held := make(chan struct{})
	var releaseOnce, heldOnce sync.Once
	releaseUpstream := func() { releaseOnce.Do(func() { close(release) }) }
	holdOpen := func() { heldOnce.Do(func() { close(held) }) }
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("content-type", "text/event-stream")
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("event: content_block_delta\ndata: {\"type\":\"content_block_delta\",\"delta\":{\"type\":\"text_delta\",\"text\":\"首块\"}}\n\n"))
		w.(http.Flusher).Flush()
		<-release
		// 客户端断开后上游仍会把终态发完：引流应当读到它。
		_, _ = w.Write([]byte("event: message_delta\ndata: {\"type\":\"message_delta\",\"delta\":{\"stop_reason\":\"end_turn\"},\"usage\":{\"output_tokens\":9}}\n\n"))
		_, _ = w.Write([]byte("event: message_stop\ndata: {\"type\":\"message_stop\"}\n\n"))
		w.(http.Flusher).Flush()
		// 写完终态后刻意不关连接：引流必须在拿到终态时就结束，而不是等上游关闭或超时。
		<-held
	}))
	// 注册顺序刻意如此：cleanup 是后进先出，必须先放行 handler 再关服务器，
	// 否则 Server.Close 会一直等在途请求，测试挂死。
	...
	result, err := ForwardStream(context.Background(), newTestPctx(t), newTestCandidate(1, "供应商甲", server.URL, 1), deps,
		StreamOptions{
			...
			Settle:      settler,
			DrainBudget: budget,
			// 引流上限远大于断言窗口：终态一到就该结束，不能靠超时兜底。
			DrainTimeout:   30 * time.Second,
			IdleTimeoutFor: func(int64) time.Duration { return 0 },
		})
	if err != nil {
		t.Fatalf("ForwardStream 失败: %v", err)
	}
	...
	stream.ClientCancel(errors.New("客户端断开"))
	// 客户端已经走了，上游这才把终态发完：引流必须继续读到 usage 与终止标记。
	releaseUpstream()
	select {
	case <-settler.settled:
	case <-time.After(2 * time.Second):
		t.Fatal("引流未在拿到终态后结束（疑似等上游关闭或走超时）")
	}

	outcome := settler.outcome()
	if deref(outcome.Observation.Usage.OutputTokens) != 9 {
		t.Fatalf("引流未取到终态用量: %+v", outcome.Observation.Usage)
	}
	if !outcome.Observation.CompletionMarker {
		t.Fatal("引流未取到终止标记")
	}
	if snapshot := budget.Snapshot(); snapshot.ActiveStreams != 0 {
		t.Fatalf("引流租约未释放: %+v", snapshot)
	}
}
