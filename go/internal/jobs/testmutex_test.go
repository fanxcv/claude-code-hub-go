...
//
// 为什么必须有：本包的价格同步整表切换（store.DeleteCloudPricesNotIn，谓词是「不在保留列表里的
// 非 manual 行」）其保留列表在调用**之前**生成，故调用瞬间之后新插入的行必然被删。共享测试库上
// 兄弟包的用例于是互相删夹具——internal/pricing 的「改价应判 updated」会拿到 `Added`，因为
// 它刚插入的行已被本包这次调用删掉。逐条把被删行补回只能把窗口缩到毫秒级，消不掉竞态；
